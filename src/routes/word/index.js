...
WordRouter.route('/(:option?)/(:word)?').get(async(req, res) => {
    let words = []
    ...
    // if(word !== "null" && word !== null){ // 데이터베이스에서 쿼리 단어를 검색
    if(word !== "undefined" && word !== undefined){
        ...
        try{
            ...
            else if(option === 'worddes'){
            words = await Word.find({$or:    // 단어 내용 검색
                [
                {r_word: {$regex:`${word}`}},
                {r_des: {$regex:`${word}`}},
                ]
    ...
    } else if (word == "null"|| word == null) { // 검색어가 존재하지 않는 경우 데이터베이스에서 전체 단어 검색
